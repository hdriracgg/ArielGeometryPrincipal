 ...
 */
public class OffsetReverseMovingShotModelNotTilted extends ReverseMovingShotModel {

    private boolean debug = false;
    private float rcvrpatchwidth;

    public OffsetReverseMovingShotModelNotTilted(GeometryModel agm, ReceiverModel arm) {
        super(agm, arm);
    }
    ...
    void buildsourcemap() {
        ...
        for (int sourceswath = 0; sourceswath < nbswaths; sourceswath++) {
            ...
            for (int sindex = 0; sindex < nbshots; sindex++) {
                ...
                if ((spnb % sspl) == 0) {
                    xcenterlocation = centerlocation.x;
                }
                float x = xcenterlocation + (sourcedirection * xoffset);
